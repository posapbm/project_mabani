...
int main()
{
	...
	if(end!=1) //showing the end of a track
	{
		printf("\n\n\n\n");
		printf("THEEND\n\n\n\n");
	}
...
int eventtype(FILE* file)
{
    ...
    if (event >= 0x80 && event < 0x90) // note off
    {
       ...
       // puts("find NOTE event");
        fread(&note, 1, 1, file);
        fread(marg, 1, 1, file);
        showtime(showtick++); // using shapeprint library to do so ;)
		return 2;
    ...
    else if (event == 0xFF) // ff event
    {
        ...
        else if (type == 0x2f) // end of track event
        {
...
double getfreq(int note){ //just turning note to its frequency
